...
| Exactly what Ethos could **not** establish | Capability declarations drive explicit downgrades; a missing capability is stated, never approximated | `capability_limits`, `evidence_tier: capability_limited` |
...
| That a **paraphrase** of a true fact is grounded | Matching is literal after whitespace normalisation only. "We may approve" and "we will approve" are different strings. | Pointer-first citation emission, so the model cites an element id and never retypes evidence (`docs/citation-emission-spec.md`) |
