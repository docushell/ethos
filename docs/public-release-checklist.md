...
publication approval decision is recorded with operator publish still pending. v0.2.0 remains the
public CLI artifact baseline with GitHub Release `v0.2.0` macOS arm64/Linux x64 artifacts, and npm
...
DocuShell integration remain blocked pending their separate operator action, registry-smoke, tag,
wording, and closeout records. The exact GitHub Release artifact closeout is limited to the
approved `v0.3.0` release assets below. Hosted surfaces, production positioning, Windows packaged
