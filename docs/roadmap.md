...
internal `ethos-core::crop_element` descriptor-only resolver and shared logical crop-ref identity
helper, but still does not add a first-class crop command, binding surface, sandbox backend, Node
beta, or MCP experimental scope.
The source-only
