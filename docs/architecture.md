...
1. **Quantize-at-extraction.** Geometry quantization happens in `ethos-pdf` before any
...
2. **Canonical payload vs envelope.** One c14n implementation in `ethos-core`; runtime
   diagnostics sit outside canonical equality; no crate hand-rolls output JSON.
3. **Backend isolation.** Only `ethos-pdf` links PDFium; `EthosPdfBackend` is the sole
...
4. **Verify portability.** `ethos-verify` compiles against the `GroundingSource` trait module
...
5. **No network in base** — three layers: (a) dependency policy (`deny.toml` bans
...
6. **License boundary in base.** `deny.toml` allows only the ADR-0004 allowlist; copyleft /
