...
- Native Ethos verification can emit deterministic, schema-backed crop descriptor JSON artifacts through `--crop-dir`; these bind `document_fingerprint`, page, bbox, and check ids. `ethos-pdf` now has a raw BGRA crop rendering boundary with byte hashing and local repeatability coverage, but the CLI still does not emit encoded image crop artifacts. The reserved `--crop-source-pdf` path validates source-PDF fingerprint binding and then fails closed until that CLI artifact path exists.
