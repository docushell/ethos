...
- Native Ethos verification can emit deterministic, schema-backed crop descriptor JSON artifacts through `--crop-dir`; these bind `document_fingerprint`, page, bbox, and check ids, but they are not rendered image crops yet. The reserved `--crop-source-pdf` path validates source-PDF fingerprint binding and then fails closed until deterministic rendering exists.
