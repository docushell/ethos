...
class MilestoneECapabilityDowngradeBoundaryRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E capability-downgrade-boundary rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `6e586f7`", text)
        self.assertIn(
        ...
        self.assertIn(
            "git grep <capability-downgrade-boundary-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <capability-downgrade-boundary-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_capability_downgrade_boundary(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("missing source capabilities", text)

    def test_make_target_runs_record_guard_after_diagnostic_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        diagnostic_row_record_guard = (
        ...
        self.assertIn(diagnostic_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(diagnostic_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(diagnostic_row_record_guard), text.index(row_record_guard))
