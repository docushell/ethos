...
class MilestoneENativeGroundingBaselineRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E native-grounding-baseline rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `eb97880`", text)
        ...
        self.assertIn("git grep <native-grounding-baseline-forbidden-wording-expression>", text)
        self.assertIn("git grep <native-grounding-baseline-record-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_covers_only_native_grounding_baseline(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("public-report blockers", text)

    def test_make_target_runs_record_guard_after_ledger_record(self) -> None:
        ...
        self.assertIn(ledger_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(ledger_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(ledger_record_guard), text.index(row_record_guard))
