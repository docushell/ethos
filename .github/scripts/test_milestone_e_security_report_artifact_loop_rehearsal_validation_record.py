...
class MilestoneESecurityReportArtifactLoopRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E security-report-artifact-loop rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `99163d0`", text)
        self.assertIn("make security-report-alpha PYTHON=<jsonschema-venv>/bin/python", text)
        self.assertIn(
        ...
        self.assertIn(
            "git grep <security-report-artifact-loop-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <security-report-artifact-loop-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_security_report_artifact_loop(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("broader security-report generation semantics", text)
        self.assertIn("artifact UX", text)

    def test_make_target_runs_record_guard_after_rag_chunk_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        rag_row_record_guard = (
        ...
        self.assertIn(rag_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(rag_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(rag_row_record_guard), text.index(row_record_guard))
