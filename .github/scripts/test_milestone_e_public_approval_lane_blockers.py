...
EXPECTED_SOURCE_SNAPSHOT = {
    ...
    "boundary": "source-snapshot-only; no public beta, package, hosted, production, or public-report approval",
}
...
EXPECTED_LANES = (
    ApprovalLane(
        ...
        "Public beta approval",
        "decider",
        "ADR-0005 does not approve public beta",
    ),
    ApprovalLane(
        2,
        "package-publication",
...
class MilestoneEPublicApprovalLaneBlockerTests(unittest.TestCase):
    def test_ledger_is_source_only_and_blocked(self) -> None:
        ...
        self.assertEqual(
            "all_public_approval_lanes_blocked_pending_dedicated_gate",
    ...
    def test_all_recommended_lanes_are_present_and_blocked(self) -> None:
        ...
        for expected, row in zip(EXPECTED_LANES, rows):
            self.assertEqual("blocked_pending_dedicated_approval", row["approval_status"])
    ...
    def test_lane_rows_keep_required_approval_contract_fields(self) -> None:
        for row in load_json(LEDGER)["approval_lanes"]:
            ...
            self.assertTrue(
                any("dedicated" in item and "approval record" in item for item in row["required_evidence"]),
                row["lane_id"],
            )
            self.assertTrue(
            ...
            self.assertIn("remains blocked", blocker_text, row["lane_id"])
    ...
    def test_status_roadmap_scope_and_validation_index_reference_lane_ledger(self) -> None:
        ...
        self.assertIn("Public beta remains blocked", status)
