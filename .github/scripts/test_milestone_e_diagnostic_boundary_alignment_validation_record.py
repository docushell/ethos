...
RECORD = ROOT / "docs/validation/milestone-e-diagnostic-boundary-alignment-validation-2026-06-20.md"
...
class MilestoneEDiagnosticBoundaryAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-diagnostic-boundary-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E diagnostic-boundary alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `d7708e4`", text)
        ...
        self.assertIn("git grep <diagnostic-boundary-alignment-forbidden-wording-expression>", text)
        self.assertIn("git grep <diagnostic-boundary-alignment-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_names_current_diagnostic_boundary_set(self) -> None:
        text = record_text()
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("pass for internal Milestone E diagnostic-boundary alignment validation", text)
        ...
        self.assertIn("diagnostic-boundary vocabulary", text)
    ...
    def test_make_target_runs_diagnostic_boundary_record_guard_in_order(self) -> None:
        block = target_block("milestone-e-prep")

        evidence_lane_record = (
