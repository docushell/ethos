...
RECORD = ROOT / "docs/validation/milestone-e-source-status-alignment-validation-2026-06-20.md"
...
class MilestoneESourceStatusAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-source-status-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E source-status alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `27b406e`", text)
        ...
        self.assertIn("git grep <source-status-alignment-forbidden-wording-expression>", text)
        self.assertIn("git grep <source-status-alignment-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_names_current_status_set(self) -> None:
        text = record_text()

        self.assertIn(f"- `{EXPECTED_TOP_LEVEL_STATUS}`", text)
        self.assertIn(f"- `{EXPECTED_CANDIDATE_ROW_STATUS}`", text)

    def test_record_keeps_source_only_internal_scope(self) -> None:
        text = normalized_record_text()

        self.assertIn("pass for internal Milestone E source-status alignment validation", text)
        ...
        self.assertIn("source-status vocabulary", text)
        self.assertIn(EXPECTED_TOP_LEVEL_STATUS, text)
        self.assertIn(EXPECTED_CANDIDATE_ROW_STATUS, text)
    ...
    def test_make_target_runs_source_status_record_guard_in_order(self) -> None:
        block = target_block("milestone-e-prep")

        promotion_status_record = (
