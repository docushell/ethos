...
class MilestoneEDemoNarrativeIndexRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E demo-narrative-index rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `72115ec`", text)
        ...
        self.assertIn(
            "git grep <demo-narrative-index-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <demo-narrative-index-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_demo_narrative_index(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("broad demo-generation", text)

    def test_make_target_runs_record_guard_after_security_report_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        security_report_row_record_guard = (
        ...
        self.assertIn(security_report_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(security_report_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(security_report_row_record_guard), text.index(row_record_guard))
