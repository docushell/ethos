...
RECORD = ROOT / "docs/validation/milestone-e-applies-to-binding-alignment-validation-2026-06-20.md"
...
class MilestoneEAppliesToBindingAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-applies-to-binding-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E applies-to binding alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `cf78e15`", text)
        ...
        self.assertIn("git grep <applies-to-binding-forbidden-wording-expression>", text)
        self.assertIn("git grep <applies-to-binding-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_names_current_binding_set(self) -> None:
        text = record_text()
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("pass for internal Milestone E applies-to binding alignment validation", text)
        ...
        self.assertIn("applies-to binding chain", text)
    ...
    def test_make_target_runs_applies_to_record_guard_in_order(self) -> None:
        ...
        self.assertLess(block.index(source_status_record), block.index(record_guard))
        self.assertLess(block.index(record_guard), block.index("git diff --check"))

    def test_ci_runs_applies_to_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(source_status_record), text.index(record_guard))
