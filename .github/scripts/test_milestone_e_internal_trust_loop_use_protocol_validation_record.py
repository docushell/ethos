...
class MilestoneEInternalTrustLoopUseProtocolValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn("internal Milestone E trust-loop use protocol validation", normalized)
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `8946185`", text)
        self.assertIn("python3 -m json.tool docs/milestone-e-internal-trust-loop-use-protocol.json", text)
        self.assertIn(
        ...
        self.assertIn("git grep <protocol-forbidden-wording-expression>", text)
        self.assertIn("git grep <protocol-record-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_keeps_source_only_internal_scope(self) -> None:
        text = normalized_record_text()

        self.assertIn("pass for internal Milestone E trust-loop use protocol validation", text)
        self.assertIn("Ethos remains source-only pre-alpha", text)
        self.assertIn("source-checkout rules for internal walkthrough use", text)
        ...
        self.assertIn("internal_trust_loop_use_protocol", text)
        self.assertIn("internal_source_only_walkthrough_use", text)
    ...
    def test_record_keeps_public_boundaries_explicit(self) -> None:
        ...
        for boundary in protocol["public_boundary"]:
            self.assertIn(boundary, lowered)
        for blocked_output in protocol["blocked_outputs"]:
            self.assertIn(blocked_output, lowered)

    def test_record_covers_every_protocol_step(self) -> None:
        text = normalized_record_text()
        protocol = json.loads(PROTOCOL.read_text(encoding="utf-8"))

        for step in protocol["protocol_steps"]:
            self.assertIn(step["step_id"], text)
            self.assertIn(step["candidate_id"], text)
            self.assertIn(step["validation_command_must_pass"], text)
            self.assertIn(step["diagnostic_boundary_must_remain"], text)
            for path in step["required_input_fixtures"]:
                self.assertIn(path, text)
            for blocker in step["blockers_must_remain_explicit"]:
                self.assertIn(blocker, text)

    def test_record_names_validated_protocol_boundary(self) -> None:
        text = normalized_record_text()

        self.assertIn("The protocol stays source-only, internal, and non-public", text)
        self.assertIn("The protocol references only current walkthrough step ids", text)
        self.assertIn("Each protocol step exactly matches its walkthrough step", text)
        self.assertIn("Each protocol step exactly matches its criteria entry", text)
        ...
        self.assertIn("Schema validation covers the protocol plan and schema", text)
        self.assertIn("Public boundaries remain explicit and blocked", text)

    def test_make_target_runs_record_guard_after_protocol_guard(self) -> None:
        ...
        self.assertLess(block.index(protocol_guard), block.index(record_guard))
        self.assertLess(block.index(walkthrough_record_guard), block.index(record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(protocol_guard), text.index(record_guard))
        self.assertLess(text.index(walkthrough_record_guard), text.index(record_guard))
