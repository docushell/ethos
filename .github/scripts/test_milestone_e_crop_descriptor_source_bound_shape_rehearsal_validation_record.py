...
class MilestoneECropDescriptorSourceBoundShapeRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E crop-descriptor-source-bound-shape rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `25e10cc`", text)
        self.assertIn(
        ...
        self.assertIn(
            "git grep <crop-descriptor-source-bound-shape-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <crop-descriptor-source-bound-shape-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_crop_descriptor_source_bound_shape(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("Node crop surfaces", text)
        self.assertIn("MCP crop surfaces", text)
        self.assertIn("hosted crop surfaces", text)
        self.assertIn("sandbox-backed crop surfaces", text)
        self.assertIn("foreign-adapter crop surfaces", text)

    def test_make_target_runs_record_guard_after_pinned_fixture_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        pinned_row_record_guard = (
        ...
        self.assertIn(pinned_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(pinned_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(pinned_row_record_guard), text.index(row_record_guard))
