...
class MilestoneERagChunkArtifactLoopRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E rag-chunk-artifact-loop rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `098ee9b`", text)
        self.assertIn("make rag-chunk-alpha PYTHON=<jsonschema-venv>/bin/python", text)
        self.assertIn(
        ...
        self.assertIn(
            "git grep <rag-chunk-artifact-loop-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <rag-chunk-artifact-loop-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_rag_chunk_artifact_loop(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("broader provenance integration", text)
        self.assertIn("broader citation integration", text)
        self.assertIn("parser integration", text)
        self.assertIn("table integration", text)

    def test_make_target_runs_record_guard_after_crop_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        crop_row_record_guard = (
        ...
        self.assertIn(crop_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(crop_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(crop_row_record_guard), text.index(row_record_guard))
