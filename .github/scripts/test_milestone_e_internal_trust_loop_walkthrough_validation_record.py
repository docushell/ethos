...
class MilestoneEInternalTrustLoopWalkthroughValidationRecordTests(unittest.TestCase):
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("internal_trust_loop_walkthrough_plan", text)
        self.assertIn("internal_source_only_walkthrough_planning", text)
    ...
    def test_record_keeps_public_boundaries_explicit(self) -> None:
        ...
        for boundary in walkthrough["public_boundary"]:
    ...
    def test_record_names_validated_walkthrough_boundary(self) -> None:
        text = normalized_record_text()
        ...
        self.assertIn("Schema validation covers the walkthrough plan and schema", text)
        self.assertIn("Public boundaries remain explicit and blocked", text)

    def test_make_target_runs_record_guard_after_criteria_record_guard(self) -> None:
        ...
        self.assertLess(block.index(walkthrough_guard), block.index(record_guard))
        self.assertLess(block.index(criteria_record_guard), block.index(record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(walkthrough_guard), text.index(record_guard))
        self.assertLess(text.index(criteria_record_guard), text.index(record_guard))
