...
class MilestoneEValidationSourceHeadAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-validation-source-head-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E validation-record source-head alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `67f1af2`", text)
        self.assertIn("python3 .github/scripts/test_milestone_e_validation_source_head_alignment.py", text)
        self.assertIn(
        ...
        self.assertIn("git grep <validation-source-head-forbidden-wording-expression>", text)
        self.assertIn("git grep <validation-source-head-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_keeps_source_only_internal_scope(self) -> None:
        text = normalized_record_text()

        self.assertIn("pass for internal Milestone E validation-record source-head alignment validation", text)
    ...
    def test_make_target_runs_validation_source_head_record_guard_in_order(self) -> None:
        ...
        self.assertLess(block.index(source_head_guard), block.index(record_guard))
        self.assertLess(block.index(record_guard), block.index(sequence_guard))
        self.assertLess(block.index(record_guard), block.index("git diff --check"))

    def test_ci_runs_validation_source_head_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(source_head_guard), text.index(record_guard))
        self.assertLess(text.index(record_guard), text.index(sequence_guard))
