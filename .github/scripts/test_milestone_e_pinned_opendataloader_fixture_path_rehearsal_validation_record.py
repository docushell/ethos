...
class MilestoneEPinnedOpenDataLoaderFixturePathRehearsalValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E pinned-opendataloader-fixture-path rehearsal validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `d58a9b0`", text)
        ...
        self.assertIn(
            "git grep <pinned-opendataloader-fixture-path-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <pinned-opendataloader-fixture-path-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_record_covers_only_pinned_opendataloader_fixture_path(self) -> None:
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("public comparison reports", text)
        self.assertIn("claim wording", text)

    def test_make_target_runs_record_guard_after_opendataloader_adapter_row_record(self) -> None:
        block = target_block("milestone-e-prep")

        adapter_row_record_guard = (
        ...
        self.assertIn(adapter_row_record_guard, block)
        self.assertIn(row_record_guard, block)
        self.assertIn(prep_record_guard, block)
        self.assertLess(block.index(adapter_row_record_guard), block.index(row_record_guard))
    ...
    def test_ci_runs_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(adapter_row_record_guard), text.index(row_record_guard))
