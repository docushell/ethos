...
RECORD = ROOT / "docs/validation/milestone-e-evidence-lane-alignment-validation-2026-06-20.md"
...
class MilestoneEEvidenceLaneAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-evidence-lane-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E evidence-lane alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `b833e4d`", text)
        ...
        self.assertIn("git grep <evidence-lane-alignment-forbidden-wording-expression>", text)
        self.assertIn("git grep <evidence-lane-alignment-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_names_current_evidence_lane_set(self) -> None:
        text = record_text()

        for lane in EXPECTED_EVIDENCE_LANES:
            self.assertIn(f"- `{lane}`", text)

    def test_record_keeps_source_only_internal_scope(self) -> None:
        text = normalized_record_text()

        self.assertIn("pass for internal Milestone E evidence-lane alignment validation", text)
        ...
        self.assertIn("evidence-lane vocabulary", text)
    ...
    def test_make_target_runs_evidence_lane_record_guard_in_order(self) -> None:
        block = target_block("milestone-e-prep")

        blocked_output_record = (
        ...
        self.assertLess(block.index(blocked_output_record), block.index(record_guard))
    ...
    def test_ci_runs_evidence_lane_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(blocked_output_record), text.index(record_guard))
