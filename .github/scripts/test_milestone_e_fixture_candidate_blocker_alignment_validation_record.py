...
CANDIDATE_SCHEMA = ROOT / "schemas/ethos-milestone-e-fixture-candidates.schema.json"
...
class MilestoneEFixtureCandidateBlockerAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        ...
        self.assertIn(
            "internal Milestone E fixture-candidate blocker alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `4c8f17f`", text)
        ...
        self.assertIn(
            "git grep <fixture-candidate-blocker-alignment-forbidden-wording-expression>",
            text,
        )
        self.assertIn(
            "git grep <fixture-candidate-blocker-alignment-record-private-path-expression>",
            text,
        )
        self.assertIn("git diff --check", text)

    def test_candidates_and_criteria_have_identical_structured_blockers(self) -> None:
        text = record_text()
    ...
    def test_schema_requires_structured_candidate_blockers(self) -> None:
        schema = load_json(CANDIDATE_SCHEMA)
        fixture_candidate = schema["$defs"]["fixture_candidate"]

        self.assertIn("blockers_must_remain_explicit", fixture_candidate["required"])
        blocker_schema = fixture_candidate["properties"]["blockers_must_remain_explicit"]
        self.assertEqual("array", blocker_schema["type"])
        self.assertEqual(1, blocker_schema["minItems"])
        self.assertTrue(blocker_schema["uniqueItems"])
    ...
    def test_record_keeps_source_only_internal_scope(self) -> None:
        ...
        self.assertIn("source-only blocker-alignment boundary", text)
    ...
    def test_make_target_runs_alignment_guard_after_criteria_guard(self) -> None:
        ...
        self.assertLess(block.index(criteria_guard), block.index(alignment_guard))
        self.assertLess(block.index(alignment_guard), block.index(walkthrough_guard))
        self.assertLess(block.index(alignment_guard), block.index("git diff --check"))

    def test_ci_runs_alignment_guard_once_in_order(self) -> None:
        ...
        self.assertIn(alignment_guard, text)
        self.assertEqual(1, text.count(alignment_guard))
        self.assertLess(text.index(criteria_guard), text.index(alignment_guard))
        self.assertLess(text.index(alignment_guard), text.index(walkthrough_guard))
