...
class ExecutionStatusTests(unittest.TestCase):
    ...
    def test_public_posture_boundary_remains_explicit(self) -> None:
        ...
        self.assertIn("Close H1: execute and review the public-safe competitor comparison flow", text)
        self.assertIn("Close H2: complete `docs/public-release-checklist.md`", text)
        self.assertIn("Run release-candidate validation gates", text)
        self.assertIn("does not close H1 or H2", text)
