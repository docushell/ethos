...
class MilestoneEPackagePublicationRegistryAssemblyActivationPrepTests(unittest.TestCase):
    def test_package_prep_artifact_records_registry_assembly_activation_prep(self) -> None:
        prep = load_json(PREP)
    ...
    def test_current_manifests_and_registry_state_stay_source_tree_only(self) -> None:
    ...
    def test_registry_assembly_activation_record_names_future_review_boundary(self) -> None:
        record = normalized(RECORD)

        self.assertIn("Validated source HEAD before this record: `622dc26`", record)
        self.assertIn(
        ...
        self.assertIn("No registry-backed assembly is activated by this record", record)
        self.assertIn("future registry-backed dependent package assembly activation review", record)
        self.assertIn("reviewed candidate package artifacts", record)
        self.assertIn("source commit and tree", record)
        self.assertIn("package dependency manifest activation approval", record)
    ...
    def test_make_and_ci_run_guard_after_manifest_activation_prep(self) -> None:
        make_block = target_block("milestone-e-prep")
        ci = read(CI_WORKFLOW)
        manifest_activation_guard = (
