...
RECORD = ROOT / "docs/validation/milestone-e-public-beta-approval-prep-validation-2026-06-20.md"
...
class MilestoneEPublicBetaApprovalPrepValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-public-beta-approval-prep-validation-2026-06-20.md", text)
        self.assertIn("internal Milestone E public beta approval prep validation", normalized)

    def test_record_names_validation_commands(self) -> None:
        text = record_text()

        self.assertIn("Validated source HEAD before this record: `d6f081a`", text)
        self.assertIn("python3 .github/scripts/test_milestone_e_public_beta_approval_prep.py", text)
        self.assertIn(
        ...
        self.assertIn("git grep <public-beta-approval-prep-forbidden-wording-expression>", text)
        self.assertIn("git grep <public-beta-approval-prep-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_keeps_public_beta_blocked(self) -> None:
        text = normalized_record_text()
        lower = text.lower()

        self.assertIn("pass for internal Milestone E public beta approval prep validation", text)
        self.assertIn("Ethos remains source-only pre-alpha", text)
        self.assertIn("public beta approval prep has started", lower)
        self.assertIn("public beta remains blocked", lower)
        self.assertIn("does not approve public beta", lower)
    ...
    def test_make_target_runs_public_beta_record_guard_in_order(self) -> None:
