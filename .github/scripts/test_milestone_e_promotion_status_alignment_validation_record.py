...
RECORD = ROOT / "docs/validation/milestone-e-promotion-status-alignment-validation-2026-06-20.md"
...
class MilestoneEPromotionStatusAlignmentValidationRecordTests(unittest.TestCase):
    def test_record_is_indexed(self) -> None:
        text = VALIDATION_README.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", text)

        self.assertIn("milestone-e-promotion-status-alignment-validation-2026-06-20.md", text)
        self.assertIn(
            "internal Milestone E promotion-status alignment validation",
            normalized,
        )
    ...
    def test_record_names_validation_commands(self) -> None:
        ...
        self.assertIn("Validated source HEAD before this record: `83b6a3a`", text)
        ...
        self.assertIn("git grep <promotion-status-alignment-forbidden-wording-expression>", text)
        self.assertIn("git grep <promotion-status-alignment-private-path-expression>", text)
        self.assertIn("git diff --check", text)

    def test_record_names_current_promotion_status(self) -> None:
        text = record_text()

        self.assertIn(f"- `{EXPECTED_PROMOTION_STATUS}`", text)

    def test_record_keeps_source_only_internal_scope(self) -> None:
        text = normalized_record_text()

        self.assertIn("pass for internal Milestone E promotion-status alignment validation", text)
        ...
        self.assertIn("promotion-status vocabulary", text)
        self.assertIn(EXPECTED_PROMOTION_STATUS, text)
    ...
    def test_make_target_runs_promotion_status_record_guard_in_order(self) -> None:
        block = target_block("milestone-e-prep")

        diagnostic_boundary_record = (
        ...
        self.assertLess(block.index(diagnostic_boundary_record), block.index(record_guard))
        self.assertLess(block.index(record_guard), block.index("git diff --check"))

    def test_ci_runs_promotion_status_record_guard_once_in_order(self) -> None:
        ...
        self.assertLess(text.index(diagnostic_boundary_record), text.index(record_guard))
