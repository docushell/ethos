# LlamaIndex RAG → Ethos

This offline example converts recorded Ethos chunks into native LlamaIndex `TextNode` and
`NodeWithScore` retrieval results, hydrates recorded structured model output with
`emit_llamaindex_citations`, and runs `ethos verify --fail-on-ungrounded`. It calls no model or
hosted service and needs no API key.
...
python3 examples/llamaindex-rag/run.py --case fabricated
```

The final command intentionally exits `1` and writes `verification-report.json` under
`target/rag-framework-examples/llamaindex/fabricated/`: verification completed and the fabricated
...
`span_refs`, and explicitly exposed `table_cells` in each node's `metadata` mapping.
