...
class RunnerTests(unittest.TestCase):
 def test_writes_bound_passing_record(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   out=root/"record.json"; r=subprocess.run(["python3",str(RUNNER),"--baseline-bin",str(bins[0]),"--candidate-bin",str(bins[1]),"--source",str(source),"--citations",str(citations),"--out",str(out)],capture_output=True,text=True)
   self.assertEqual(0,r.returncode,r.stderr); record=json.loads(out.read_text()); self.assertTrue(record["derived"]["passed"]); self.assertEqual(30,len(record["single_request_cold_ns"]["baseline"])); self.assertEqual(32,len(record["batch_32_ns"]["individual_process"]))
