...
def validate(record: object, baseline: Path | None = None, candidate: Path | None = None) -> dict[str, object]:
...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--record", required=True, type=Path); parser.add_argument("--baseline-bin", type=Path); parser.add_argument("--candidate-bin", type=Path); args = parser.parse_args()
    try: result = validate(json.loads(args.record.read_text(encoding="utf-8")), args.baseline_bin, args.candidate_bin)
